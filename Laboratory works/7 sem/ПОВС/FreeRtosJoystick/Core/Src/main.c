...
int main(void)
{
  ...
  osThreadDef(defaultTask, defaultTaskFunc, osPriorityRealtime, 0, 128);
  defaultTaskHandle = osThreadCreate(osThread(defaultTask), NULL);

  /* definition and creation of JsUartTask */
  osThreadDef(JsUartTask, JsUartTaskFunc, osPriorityRealtime, 0, 128);
  JsUartTaskHandle = osThreadCreate(osThread(JsUartTask), NULL);

  /* definition and creation of BtnUartTask */
  osThreadDef(BtnUartTask, BtnUartTaskFunc, osPriorityRealtime, 0, 128);
  ...
  for(;;)
  {
    ...
    if (cnt != 0)
    {
    	for (int i = 0; i < 8; i++)
    	{
    		for (int j = 0; j < 4; j++)
    		{
    			js[1 + 4*i + j] = (js_buf[i] >> (4 * (3 - j))) & 0xFF;
    		}
    	}
...
void BtnUartTaskFunc(void const * argument)
{
  ...
  for(;;)
  {
	  ...
	  if (!HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10)) { btns[1] = checked; cnt++; }
	  if (!HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_3)) { btns[2] = checked; cnt++; }
	  if (!HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_5)) { btns[3] = checked; cnt++; }
	  if (!HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_4)) { btns[4] = checked; cnt++; }
	  if (!HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_10)) { btns[5] = checked; cnt++; }
	  if (!HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_8)) { btns[6] = checked; cnt++; }
	  if (!HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_9)) { btns[7] = checked; cnt++; }

	  if (cnt != 0)
		  HAL_UART_Transmit(&huart2, btns, sizeof(btns), 500);
	  //the last argument is timeout, for what does it here???
  }
