...
class Notes extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:8080/notes')
            .then(responce => {
                this.setState({notes: responce.data});
            });
    }
            ...
            <>
                ...
                <div className="px-4 mb-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                    {this.state.notes.filter(note => {
                        return note.title.toLowerCase().match(this.state.searchQuery.toLowerCase()) ||
                            note.body.toLowerCase().match(this.state.searchQuery.toLowerCase());
                    }).map(note => (
